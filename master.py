...
def read_csv_safely(path: str, required_cols: list[str], file_label: str) -> pd.DataFrame:
    file_path = Path(path)
    ...
    try:
        ...
        st.error(f"{file_label} CSV 형식이 깨져 있습니다.")
        st.caption(f"현재 앱이 읽는 경로: {file_path.resolve()}")
        st.caption("특정 줄의 콤마 개수가 맞지 않거나, 코드블록 표시(```csv)가 같이 저장된 경우가 많습니다.")
        return pd.DataFrame(columns=required_cols)
    except Exception as exc:
        st.error(f"{file_label} 파일을 읽는 중 오류가 발생했습니다: {exc}")
        st.caption(f"현재 앱이 읽는 경로: {file_path.resolve()}")
        return pd.DataFrame(columns=required_cols)


def is_korea_ticker_series(ticker_series: pd.Series) -> pd.Series:
...
def normalize_peer_master(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # master 입력값은 유지하고, 비어 있을 때만 기본값 보정
...
def normalize_financials(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[(df["Ticker"] != "") & (df["Period"] != "")].copy()
    return df


@st.cache_data
def load_peer_master(file_mtime=None) -> pd.DataFrame:
    raw_df = read_csv_safely(PEER_MASTER_PATH, REQUIRED_PEER_COLS, "peer_master.csv")
    return normalize_peer_master(raw_df)


@st.cache_data
def load_financials(file_mtime=None) -> pd.DataFrame:
    raw_df = read_csv_safely(FINANCIALS_PATH, REQUIRED_FINANCIAL_COLS, "financials.csv")
    return normalize_financials(raw_df)
...
def pick_from_statement(df: pd.DataFrame, possible_names: list[str]):
...
def get_latest_yfinance_financials(ticker: str) -> dict:
    ...
    revenue = pick_from_statement(income_stmt, ["Total Revenue", "Operating Revenue", "Revenue"])
    ebitda = pick_from_statement(income_stmt, ["EBITDA", "Normalized EBITDA"])
    net_income = pick_from_statement(income_stmt, ["Net Income", "Net Income Common Stockholders"])

    cash = pick_from_statement(
        balance_sheet,
        ["Cash And Cash Equivalents", "Cash Cash Equivalents And Short Term Investments"],
    )
    total_debt = pick_from_statement(balance_sheet, ["Total Debt"])

    if pd.isna(total_debt):
        short_debt = pick_from_statement(
            balance_sheet,
            ["Current Debt", "Current Debt And Capital Lease Obligation"],
        )
        long_debt = pick_from_statement(
            balance_sheet,
...
def style_valuation_table(df: pd.DataFrame):
    ...
    existing_format = {col: fmt for col, fmt in format_dict.items() if col in df.columns}
    ...
    if DART_API_KEY is None:
        ...
        if korea_peer_df.empty:
            ...
            with st.spinner("DART 재무데이터를 수집하는 중입니다..."):
                dart_financials_df, dart_logs_df = fetch_dart_financials_for_korea_peers(default_peer_df, DART_API_KEY, bsns_year=dart_year)
...
peer_df = normalize_peer_master(default_peer_df.copy())
...
with st.form("peer_selection_form"):
    ...
    with col1:
        selected_period_input = st.selectbox(
            "기준 실적 기간",
            available_periods,
            index=available_periods.index(st.session_state.get("selected_period_saved", available_periods[0])) if st.session_state.get("selected_period_saved", available_periods[0]) in available_periods else 0,
        )

    with col2:
        multiple_options = ["EV/EBITDA", "EV/Revenue", "P/E"]
        selected_multiple_type_input = st.selectbox(
            "Multiple 기준",
            multiple_options,
            index=multiple_options.index(st.session_state.get("selected_multiple_type_saved", "EV/EBITDA")) if st.session_state.get("selected_multiple_type_saved", "EV/EBITDA") in multiple_options else 0,
        )

    st.write("Peer 기업 선택")
    peer_select_df = peer_df.copy()
    peer_select_df["Use"] = peer_select_df["Ticker"].isin(st.session_state["selected_peer_tickers"])

    peer_select_display_cols = ["Use", "Ticker", "Company", "Peer Group", "Country", "Category"]
    ...
    edited_peer_df = st.data_editor(
        ...
        disabled=["Ticker", "Company", "Peer Group", "Country", "Category"],
...
if run_calculation:
    selected_peer_tickers = edited_peer_df.loc[edited_peer_df["Use"] == True, "Ticker"].tolist()
    st.session_state["selected_peer_tickers"] = selected_peer_tickers

    filtered_peer_df = peer_df[peer_df["Ticker"].isin(selected_peer_tickers)].copy()
    ...
    valuation_df = calculate_peer_valuation(filtered_peer_df, financial_df, market_df, selected_period_input)

    st.session_state["valuation_ready"] = True
    st.session_state["valuation_df"] = valuation_df
    st.session_state["selected_period_saved"] = selected_period_input
    st.session_state["selected_multiple_type_saved"] = selected_multiple_type_input

if not st.session_state["valuation_ready"]:
    st.stop()

valuation_df = st.session_state.get("valuation_df", pd.DataFrame())
selected_period = st.session_state.get("selected_period_saved", available_periods[0])
selected_multiple_type = st.session_state.get("selected_multiple_type_saved", "EV/EBITDA")
...
with st.form("target_company_form"):
    ...
    with target_col3:
        basis_options = ["Average", "Median", "Manual"]
        selected_basis = st.radio(
            "적용 Multiple",
            basis_options,
            index=basis_options.index(st.session_state.get("selected_basis", "Median")) if st.session_state.get("selected_basis", "Median") in basis_options else 1,
            horizontal=True,
        )
...
if fetch_target:
    if target_ticker.strip() == "":
        ...
        try:
            ...
            st.error(f"Target Company 데이터를 가져오지 못했습니다: {exc}")
...
if target_data is not None:
    ...
    st.dataframe(style_valuation_table(target_summary_df), use_container_width=True)

    result_col1, result_col2, result_col3, result_col4 = st.columns(4)
    result_col1.metric("적용 Multiple", f"{applied_multiple:,.1f}x")
    result_col2.metric(target_metric_name, f"{target_metric:,.1f}M" if pd.notna(target_metric) else "N/A")
    result_col3.metric("Implied EV", f"{implied_ev:,.1f}M" if pd.notna(implied_ev) else "N/A")
    result_col4.metric("Implied Equity", f"{implied_equity:,.1f}M" if pd.notna(implied_equity) else "N/A")
...
if target_data is None:
    ...
    if pd.isna(target_metric):
        ...
        st.dataframe(style_valuation_table(sensitivity_df), use_container_width=True)

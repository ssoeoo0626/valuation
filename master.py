...
def read_csv_safely(path: str, required_cols: list[str], file_label: str) -> pd.DataFrame:
    ...
    if not file_path.exists():
        st.warning(f"{file_label} 파일을 찾을 수 없습니다: {file_path.resolve()}")
...
def normalize_peer_master(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # master 값을 강제로 덮지 않고, 비어 있을 때만 보정
...
def load_peer_master(file_mtime=None) -> pd.DataFrame:
    peer_df = read_csv_safely(PEER_MASTER_PATH, REQUIRED_PEER_COLS, "peer_master.csv")
    return normalize_peer_master(peer_df)


@st.cache_data
def load_financials(file_mtime=None) -> pd.DataFrame:
    financial_df = read_csv_safely(FINANCIALS_PATH, REQUIRED_FINANCIAL_COLS, "financials.csv")
...
def pick_amount(fs_df: pd.DataFrame, keywords: list[str], sj_div=None):
    ...
    for keyword in keywords:
        matched = temp[
            temp["account_nm"].astype(str).str.contains(keyword, na=False, regex=False)
        ]
...
def pick_from_yf_df(df: pd.DataFrame, possible_names: list[str]):
...
with col1:
    selected_period = st.selectbox("기준 실적 기간", available_periods, key="period_selector")
with col2:
    selected_multiple_type = st.selectbox("Multiple 기준", ["EV/EBITDA", "EV/Revenue", "P/E"], key="multiple_selector")

st.write("Peer 기업 선택")

peer_select_df = peer_df.copy()
if "selected_peer_tickers" not in st.session_state:
    default_selected_tickers = peer_select_df.loc[
        peer_select_df["Peer Group"].isin(["CG/VFX", "Global CG/VFX"]),
        "Ticker",
    ].tolist()
    if not default_selected_tickers:
        default_selected_tickers = peer_select_df["Ticker"].tolist()
    st.session_state["selected_peer_tickers"] = default_selected_tickers
...
st.caption(f"선택된 Peer: {len(selected_peer_tickers):,}개")
...
with st.expander("Multiple 산정 대상 Peer 보기"):
    st.dataframe(
        multiple_df_for_summary[["Ticker", "Company", "Category", "Peer Group", selected_multiple_type]].style.format({
...
with st.form("target_company_form"):
    ...
    with target_col3:
        saved_basis = st.session_state.get("selected_basis", "Median")
        selected_basis = st.radio(
            "적용 Multiple",
            ["Average", "Median", "Manual"],
            index=["Average", "Median", "Manual"].index(saved_basis),
            horizontal=True,
        )
...
if target_data is not None:
    selected_basis_for_calc = st.session_state.get("selected_basis", "Median")

    if selected_basis_for_calc == "Average":
        applied_multiple = avg_multiple
    elif selected_basis_for_calc == "Median":
        applied_multiple = median_multiple
    else:

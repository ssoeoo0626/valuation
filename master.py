...
def read_csv_safely(path, required_cols, file_label):
    file_path = Path(path)
    ...
    try:
        ...
        st.caption("대부분 특정 줄의 콤마 개수가 맞지 않거나, 코드블록 표시(```csv)가 같이 저장된 경우입니다.")
        return pd.DataFrame(columns=required_cols)
